# LockBit — Splunk Hunting Searches
...
The `index=edr` examples use an explicitly normalized event schema. Map your EDR data to `event_type` (process/file/network/registry), `action` (created/renamed/modified/connected/value_set), `host`, `user`, `process_name` (basename), `process_path`, `process_command_line`, `parent_process` (basename), `process_id`, `process_start_time`, `file_name`, `file_path` (complete path), `previous_file_name`, `sha256`, `dest_ip`, `dest_port`, `dest_domain`, `registry_path` (key without value name), `registry_value_name` and `registry_value_data`. Version-metadata hunts additionally require `company_name`, `product_name`, `original_file_name` and `file_description`.

These fields are a documented adapter contract, not a claim that every vendor or Splunk CIM deployment supplies them unchanged. L19 instead uses forwarded Windows events in `index=windows`; L36 uses ESXi logs in `index=esxi`. Set index names and field aliases to the local deployment before execution. Sysmon file-creation events alone do not provide complete rename or modification coverage.
...
|---|---|---|
| 1. Credential Access | 4 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 2. Active Directory and Network Discovery | 3 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 3. Persistence and Remote Administration | 4 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 4. Tunneling and C2 | 3 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 5. Defense Evasion and Impairment | 6 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 6. Collection and Exfiltration | 3 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 7. Recovery Inhibition | 2 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 8. Deployment and Impact | 4 searches | Per-entry source, telemetry and tuning; correlate before attribution |
| 9. Multi-Stage Correlation | 1 search | Per-entry source, telemetry and tuning; correlate before attribution |
| 10. Campaign Artifact Hunts | 10 searches | Per-entry source, telemetry and tuning; correlate before attribution |
...
```spl
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="rundll32.exe" AND like(cmd,"%comsvcs.dll%") AND like(cmd,"%minidump%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"mimikatz.exe","lazagne.exe","passwordfox.exe","extpassword.exe") OR like(cmd,"%sekurlsa::%") OR like(cmd,"%lsadump::%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="reg.exe" AND match(cmd,"\\bsave\\b") AND (like(cmd,"%hklm%sam%") OR like(cmd,"%hklm%system%") OR like(cmd,"%hkey_local_machine%sam%"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
```spl
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where (name="a.png" AND like(path,"%users%public%")) OR (in(name,"a.cab","am.cab","em.cab","z.txt") AND like(path,"%windows%tasks%"))
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"adfind.exe","sharphound.exe","bloodhound.exe","seatbelt.exe") OR like(cmd,"%sharphound%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where (in(image,"net.exe","net1.exe") AND match(cmd,"\\b(group|view|user)\\b")) OR in(image,"nltest.exe","quser.exe","whoami.exe")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
```spl
index=edr earliest=-7d event_type=network action=connected dest_port IN (445,3389,5985,5986)
| where isnotnull(process_start_time)
| bin _time span=5m
| stats dc(dest_ip) AS Targets values(dest_ip) AS Addresses by host process_name process_id process_start_time _time
| where Targets>=20
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"psexec.exe","psexec64.exe","psexesvc.exe","wmiexec.exe") OR lower(parent_process)="wmiprvse.exe"
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="schtasks.exe" AND (like(cmd,"%/create%") OR like(cmd,"%updateadobetask%") OR like(cmd,"%megacmd%"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"anydesk.exe","anydeskmsi.exe","teamviewer.exe","ateraagent.exe","srutility.exe") OR like(image,"screenconnect%") OR like(lower(product_name),"%screenconnect%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
```spl
index=edr earliest=-7d event_type=registry action=value_set
| eval key=lower(coalesce(registry_path,"")), value=lower(coalesce(registry_value_data,""))
| where match(key,"currentversion\\\\run(once)?$") AND (like(value,"%temp%") OR like(value,"%users%public%"))
| table _time host registry_path registry_value_name registry_value_data process_name process_command_line
```
...
```spl
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,""))
| where in(image,"plink.exe","ssh.exe","servicehost.exe") AND match(process_command_line,"(?:^|\\s)-[RLD](?:\\s|\\d)")
| table _time host user process_name process_path process_command_line parent_process
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"ngrok.exe","ligolo.exe","ligolo-ng.exe") OR (like(cmd,"%ngrok%") AND match(cmd,"\\b(tcp|http|start)\\b"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
```spl
index=edr earliest=-7d event_type=network dest_ip IN ("81.19.135.219","81.19.135.220","81.19.135.226","193.201.9.224","62.233.50.25","168.100.9.137","206.188.197.22","141.98.9.137")
| table _time host dest_ip dest_port dest_domain process_name process_command_line action
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="dllhost.exe" AND (like(cmd,"%3e5fc7f9-9a51-4367-9063-a120244fbec7%") OR like(cmd,"%d2e7041b-2927-42fb-8e9f-7ce93b6dc937%"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"powershell.exe","pwsh.exe") AND (like(cmd,"%set-mppreference%") OR like(cmd,"%add-mppreference%")) AND (like(cmd,"%disablerealtimemonitoring%") OR like(cmd,"%exclusionpath%") OR like(cmd,"%exclusionprocess%"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="bcdedit.exe" AND match(cmd,"\\bsafeboot\\b") AND like(cmd,"%/set%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"sc.exe","net.exe","net1.exe","powershell.exe","pwsh.exe") AND match(cmd,"\\b(stop|stop-service|set-service|config)\\b") AND match(cmd,"\\b(vss|sqlwriter|veeam|windefend|wbengine)\\b")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
| eval eid=tonumber(coalesce(EventCode,EventID)), provider=lower(coalesce(Provider_Name,ProviderName,SourceName,"")), channel=lower(coalesce(Channel,LogName,""))
| where (eid=1102 AND channel="security") OR (eid=104 AND provider="microsoft-windows-eventlog")
| table _time host eid channel provider Message
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where in(image,"processhacker.exe","gmer.exe","pchunter64.exe","tdsskiller.exe","backstab.exe","defendercontrol.exe","powertool.exe")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where (in(image,"rclone.exe","rclone") AND match(cmd,"\\b(copy|sync|move)\\b")) OR in(image,"megacmd.exe","mega-put.exe","megasync.exe")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where (in(image,"7z.exe","7za.exe") AND match(cmd,"(?:^|\\s)a(?:\\s|$)")) OR (image="makecab.exe" AND (like(cmd,"%windows%tasks%") OR like(cmd,"%users%public%")))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where like(image,"stealbit%") OR like(lower(original_file_name),"stealbit%")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where (image="vssadmin.exe" AND match(cmd,"\\bdelete\\b") AND match(cmd,"\\bshadows\\b")) OR (image="wmic.exe" AND match(cmd,"\\bshadowcopy\\b") AND match(cmd,"\\bdelete\\b"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where (image="wbadmin.exe" AND match(cmd,"\\bdelete\\b")) OR (image="bcdedit.exe" AND (like(cmd,"%recoveryenabled%") OR like(cmd,"%bootstatuspolicy%")))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where (like(path,"%sysvol%") OR like(path,"%netlogon%")) AND (like(name,"%.exe") OR like(name,"%.dll"))
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where name="restore-my-files.txt" OR like(name,"%.lockbit") OR match(name,"^[a-z0-9]{9}\\.readme\\.txt$")
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
```spl
index=edr earliest=-7d event_type=file action=renamed
| where match(file_name,"(?i)\\.[a-f0-9]{16}$") AND NOT match(coalesce(previous_file_name,""),"(?i)\\.[a-f0-9]{16}$")
| where isnotnull(process_start_time)
| bin _time span=5m
| stats dc(file_path) AS Files values(file_name) AS Examples by host process_name process_id process_start_time _time
| where Files>=25
```
...
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where name="readmefordecrypt.txt" OR match(name,"\\.[a-f0-9]{16}$")
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
```spl
index=edr earliest=-1d (event_type=process OR (event_type=file action=created))
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,"")), name=lower(coalesce(file_name,""))
| eval recovery=if(event_type="process" AND ((image="vssadmin.exe" AND match(cmd,"\\bdelete\\b") AND match(cmd,"\\bshadows\\b")) OR (image="wbadmin.exe" AND match(cmd,"\\bdelete\\b"))),1,0)
| eval note=if(event_type="file" AND (in(name,"readmefordecrypt.txt","restore-my-files.txt") OR match(name,"^[a-z0-9]{9}\\.readme\\.txt$")),1,0)
| where recovery=1 OR note=1
| sort 0 host _time
| streamstats current=f last(eval(if(recovery=1,_time,null()))) AS RecoveryTime last(eval(if(recovery=1,cmd,null()))) AS RecoveryCommand by host
| where note=1 AND isnotnull(RecoveryTime) AND _time>=RecoveryTime AND _time-RecoveryTime<=1800
| table _time host RecoveryTime RecoveryCommand file_name file_path process_name
```
...
```spl
index=edr earliest=-30d event_type IN ("process","file")
| eval digest=lower(coalesce(sha256,""))
| where in(digest,"7ea5afbc166c4e23498aa9747be81ceaf8dad90b8daa07a6e4644dc7c2277b82","180e93a091f8ab584a827da92c560c78f468c45f2539f73ab2deb308fb837b38","4dc06ecee904b9165fa699b026045c1b6408cc7061df3d2a7bc2b7b4f0879f4d","90b06f07eb75045ea3d4ba6577afc9b58078eafeb2cdd417e2a88d7ccf0c0273","98d8c7870c8e99ca6c8c25bb9ef79f71c25912fbb65698a9a6f22709b8ad34b6","6d0166d181db1f6c381c3ff5fff4fe4106fbc17bc29316e3cf3f65136ee4803c")
| table _time host event_type process_name process_path file_name file_path digest
```
...
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where like(name,"%.locked_for_lockbit")
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
index=edr earliest=-7d event_type=file action IN ("created","renamed","modified")
| eval name=lower(coalesce(file_name,"")), path=lower(coalesce(file_path,""))
| where in(name,"mag.dll","adobelib.dll","123.ps1") AND (like(path,"%users%public%") OR like(path,"%temp%"))
| table _time host action file_name file_path process_name process_id process_start_time sha256
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where image="fsutil.exe" AND match(cmd,"\\bfile\\b") AND match(cmd,"\\bsetzerodata\\b")
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
...
```spl
index=esxi earliest=-7d "vim-cmd"
| where like(_raw,"%getallvms%") OR like(_raw,"%power.off%")
...
index=edr earliest=-30d event_type IN ("process","file")
| eval digest=lower(coalesce(sha256,""))
| where in(digest,"0447c931bb8efc6dc531f69a891f2a0f28a85a18b25e04366fdb59bf827b2eb1","31208a2640c1f2806d21bb8b40abd47b24dd3be85dedb1fdb9f33dac47b23152","9b5f1ec1ca04344582d1eca400b4a21dfff89bc650aba4715edd7efb089d8141","b3a994f26b694fcfdc68e57fc6aeea2aa4b4906ff50b0319e00c693537a3b25c","f8935a295a316e15f60fadf465383f19cf881a42ba008ed1792cbeecb21580dc")
| table _time host event_type process_name process_path file_name file_path digest
```
...
index=edr earliest=-7d event_type=process
| eval image=lower(coalesce(process_name,"")), cmd=lower(coalesce(process_command_line,""))
| where lower(company_name)="corteva" AND (like(lower(product_name),"%solstice google photos%") OR like(lower(file_description),"%solstice google photos%"))
| table _time host user process_name process_path process_command_line process_id process_start_time parent_process sha256
```
